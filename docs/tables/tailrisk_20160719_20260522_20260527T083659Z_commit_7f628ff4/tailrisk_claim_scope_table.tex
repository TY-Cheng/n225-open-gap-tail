...
\midrule
benchmark\_metrics.parquet & baseline benchmark set & yes after clean-run author review \\
...
ml\_tail\_metrics\_per\_model.parquet & per-model out-of-sample diagnostics & coverage-screen inputs only; raw rows are not a cross-model comparison \\
ml\_tail\_result\_matrix*.parquet & restricted model-family and increment comparisons & no; restricted sample evidence only \\
canonical forecasts + coverage-screen table & coverage-admissible post-screen FZ DM & yes; conditional on the screen and strict common dates \\
\midrule
