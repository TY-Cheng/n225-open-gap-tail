...
Modeling | Benchmark historical, rolling, vol-scaled, GARCH/GJR, and GJR-EVT baseline floor runs behind gates. Advanced CAViaR, CARE/expectile, Taylor ALD, direct FZ-loss, GAS-t, and GAS-POT benchmark families are implemented as nonblocking forecast rows plus optimizer diagnostics. ML tail LightGBM direct-quantile, fully OOF location-scale, and standardized-loss POT-GPD run the registered information-set ladder after the leakage gate passes for both left-tail and right-tail risk surfaces. The headline ladder stays in `ml_tail_metrics.parquet`; restricted VaR-only and VaR-ES model-family comparisons live in `ml_tail_result_matrix*` artifacts. Block-bootstrap DM, HLN Tmax MCS, side-specific instrumented conditional predictive ability diagnostics, Murphy, stress-window, feature-unavailability, ES severity, diagnostic VaR-trigger, and DST attenuation tables are artifact-level outputs with explicit claim boundaries. | DST formal mechanism tests, hedge PnL/cost analysis, and additional robustness model variants remain future work until explicitly implemented and audited.
