...
from n225_open_gap_tail.reporting.latex_headline import (  # noqa: E402
    PROMOTED_TAIL_MODEL_SPECS,
    SELECTED_MODEL_COVERAGE_TOLERANCE,
    SELECTED_MODEL_MIN_ROWS,
    _promoted_tail_model_rows,
)
...
def _metric_row(metric: Mapping[str, object], *, group: str) -> dict[str, object]:
    ...
    breach_pass = coverage_error is not None and coverage_error <= SELECTED_MODEL_COVERAGE_TOLERANCE
    ...
    return {
        ...
        "gate_pass": breach_pass and n_rows >= SELECTED_MODEL_MIN_ROWS,
...
def _audit_markdown(*, run_id: str, full_metrics: pl.DataFrame) -> str:
    ...
    sections = [
        ...
        (
            ...
            f"{SELECTED_MODEL_COVERAGE_TOLERANCE}`; gate pass additionally requires "
            f"`N >= {SELECTED_MODEL_MIN_ROWS}`."
        ),
        "",
        ...
        "",
        (
            "These are the locked, pre-specified promoted rows used in the current "
            "slides, not a universal best-FZ ranking."
        ),
        "",
        _slide_promoted_table(lgbm),
        "",
...
def _slide_promoted_table(frame: pl.DataFrame) -> str:
    metric_like = _metrics_for_promoted_helper(frame)
    rows: list[tuple[object, ...]] = []
    for row in _promoted_tail_model_rows(metric_like):
        rows.append(
            (
                row.get("promotion_role", "n/a"),
                display_model_label(row.get("model_name")),
                display_information_set_label(row.get("information_set")),
                _tail_label(row.get("tail_side")),
                int(_optional_float(row.get("rows")) or 0),
                _fmt_rate(row.get("var_breach_rate")),
                "Y" if row.get("promotion_status") == "pass" else "N",
                _fmt_float(row.get("mean_quantile_loss")),
                _fmt_float(row.get("mean_fz_loss")),
                _fmt_float(row.get("mean_exceedance_severity")),
            )
        )
    return _markdown_table(
...
def _metrics_for_promoted_helper(frame: pl.DataFrame) -> pl.DataFrame:
    records: list[dict[str, object]] = []
    for spec in PROMOTED_TAIL_MODEL_SPECS:
        side = _tail_label(spec.get("tail_side"))
        info = display_information_set_label(spec.get("information_set"))
        model = display_model_label(spec.get("model_name"))
        match = frame.filter(
            (pl.col("tail") == side) & (pl.col("info") == info) & (pl.col("model") == model)
        )
        if match.is_empty():
            continue
        row = dict(match.row(0, named=True))
        records.append(
            {
                ...
                "rows": row["N"],
                "var_breach_rate": row["breach"],
                "expected_breach_rate": 0.05,
                "mean_quantile_loss": row["q_loss"],
                "mean_fz_loss": row["fz_loss"],
                "mean_exceedance_severity": row["severity"],
            }
        )
